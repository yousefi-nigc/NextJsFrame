...
// -------- واحدهای خروج (x) --------
// =================== محاسبات زمان تخلیه (t) - FRAME 2015 ===================

/**
 * محاسبه تعداد واحدهای خروج (x)
 * هر واحد = 0.6 متر عرض مفید
 */
/**
 * تابع اصلی محاسبه ضریب زمان تخلیه (t)
 * مطابق فرمول FRAME 2015
 */

/**
 * محاسبه تعداد واحدهای خروج (x)
 * اولویت: ورود دستی > محاسبه خودکار از عرض درها
 */
/**
// =================== اصلاحات اساسی برای محاسبه t ===================

/**
 * محاسبه تعداد واحدهای خروج (x)
 * بر اساس FRAME 2015 صفحه 18
 */
// =================== محاسبه واحدهای خروج (x) ===================
// محاسبه x - تعداد واحدهای خروج
// =====================================================
// محاسبه x - تعداد واحدهای خروج (FRAME 2015)
// =====================================================
// =================== محاسبه واحدهای خروج (x) ===================
function calculateExitUnits() {
    console.log('🔵 calculateExitUnits() شروع شد');
    
    // 1️⃣ بررسی ورودی دستی
    const manualX = document.getElementById('manual-x-input')?.value;
    if (manualX && manualX.trim() !== '') {
        const x = parseFloat(manualX);
        if (x > 0) {
            calculationResults.x = x;
            console.log('✅ x دستی:', x);
            updateDisplay('x-result', 'x', x);
            return x;
        }
    }

    // 2️⃣ دریافت عرض خروجی‌ها (به سانتی‌متر)
    const widthsInput = document.getElementById('exit-widths')?.value;
    console.log('📥 ورودی exit-widths:', widthsInput);
    
    if (!widthsInput || widthsInput.trim() === '') {
        console.log('⚠️ هیچ ورودی‌ای برای exit-widths وجود ندارد');
        return null;
    }

    // 3️⃣ تبدیل به آرایه اعداد (فرض: ورودی به سانتی‌متر است)
    const widths = widthsInput
        .split(',')
        .map(w => parseFloat(w.trim()))
        .filter(w => !isNaN(w) && w > 0);
    
    console.log('📊 عرض‌های استخراج شده (cm):', widths);
    
    if (widths.length === 0) {
        showMessage('error', 'لطفاً عرض خروجی‌ها را به سانتی‌متر وارد کنید (مثال: 80, 80)');
        return null;
    }

    // 4️⃣ محاسبه x برای هر خروجی
    let totalX = 0;
    widths.forEach((width_cm, index) => {
        const effectiveWidth = width_cm - 20;
        const units = effectiveWidth > 0 ? Math.floor(effectiveWidth / 60) : 0;
        totalX += units;
        console.log(`   خروجی ${index + 1}: ${width_cm}cm → عرض مفید: ${effectiveWidth}cm → واحدها: ${units}`);
    });

    totalX = Math.max(totalX, 1);
    console.log('🎯 مجموع x محاسبه شده:', totalX);

    // 5️⃣ ذخیره و نمایش
    calculationResults.x = totalX;
    
    updateDisplay('x-result', 'x', totalX, `
        <div style="margin-top:0.5rem;font-size:0.9rem;color:#607d8b;">
            تعداد خروجی‌ها: ${widths.length}<br>
            عرض‌های وارد شده (cm): ${widths.join(', ')}<br>
            فرمول: x = ⌊(width - 20) / 60⌋
        </div>
    `);

    console.log('✅ calculationResults.x ذخیره شد:', calculationResults.x);
    return totalX;
}

// =================== محاسبه K (مسیرهای مجزا) ===================
function calculateKfromPaths() {
    const manualK = parseFloat(document.getElementById('manual-k-input')?.value);
    if (!isNaN(manualK) && manualK > 0) {
        calculationResults.K = Math.min(Math.floor(manualK), 4);
        const resultDiv = document.getElementById('k-result');
        if (resultDiv) {
            resultDiv.innerHTML = `<strong>K (مسیرهای مجزا - دستی):</strong> ${calculationResults.K}`;
        }
        console.log('✅ K (دستی) =', calculationResults.K);
        return calculationResults.K;
    }

    const O = parseFloat(document.getElementById('external-exits')?.value) || 0;
    const x = calculationResults.x || calculateExitUnits();
    const X = parseFloat(document.getElementById('occupants-count')?.value) || 0;
    
    if (x <= 0 || X <= 0 || O <= 0) {
        calculationResults.K = 1;
        console.log('⚠️ مقادیر ناکافی، K = 1');
        return 1;
    }

    const totalCapacity = x * 120;
    const ratio = totalCapacity / X;
    const theoreticalK = Math.min(ratio, 4);
    const K = Math.floor(Math.min(O, theoreticalK));
    calculationResults.K = Math.max(1, Math.min(K, 4));
    
    const resultDiv = document.getElementById('k-result');
    if (resultDiv) {
        resultDiv.innerHTML = `
            <strong>K (مسیرهای مجزا):</strong> ${calculationResults.K}<br>
            <small style="color:#607d8b">
            گام 1: O=${O} | 
            گام 2: ظرفیت=${totalCapacity} نفر/دقیقه | 
            گام 3: نسبت=${ratio.toFixed(2)} → K_نظری=${theoreticalK.toFixed(2)} → 
            K=min(${O}, floor(${theoreticalK.toFixed(2)})) = ${calculationResults.K}
            </small>
        `;
    }
    
    console.log('✅ calculateKfromPaths: K =', calculationResults.K);
    return calculationResults.K;
}

// =================== محاسبه ضریب تحرک (p) - نسخه نهایی ===================

/**
 * محاسبه p میانگین برای گروه مختلط
 */
function calculateCustomP() {
    const pA = 1, pB = 2, pC = 8, pD = 20;
    const percentA = parseFloat(document.getElementById('p-percent-a')?.value) || 0;
    const percentB = parseFloat(document.getElementById('p-percent-b')?.value) || 0;
    const percentC = parseFloat(document.getElementById('p-percent-c')?.value) || 0;
    const percentD = parseFloat(document.getElementById('p-percent-d')?.value) || 0;
    
    const total = percentA + percentB + percentC + percentD;
    
    if (Math.abs(total - 100) > 0.1) {
        document.getElementById('custom-p-value').innerHTML = 
            `<span style="color: red;">⚠️ مجموع درصدها باید 100 باشد (فعلی: ${total}%)</span>`;
        return null;
    }
    
    const pWeighted = (pA * percentA + pB * percentB + pC * percentC + pD * percentD) / 100;
    
    document.getElementById('custom-p-value').innerHTML = 
        `<span style="color: #4caf50;">✅ p میانگین = ${pWeighted.toFixed(2)}</span>`;
    
    calculationResults.p_base = pWeighted;
    
    // ⭐ محاسبه با شرایط اضافی
...
/**
 * محاسبه ضریب تحرک نهایی (با شرایط اضافی)
 */
function calculateMobilityFactor() {
    console.log('🔵 محاسبه ضریب تحرک (p) شروع شد');

    // 1️⃣ دریافت مقدار پایه
    let baseMobility;
    const selectValue = document.getElementById('mobility-factor')?.value;

    if (selectValue === 'custom') {
        // استفاده از مقدار محاسبه‌شده قبلی
        baseMobility = calculationResults.p_base;
        
        if (!baseMobility) {
            // اگر هنوز محاسبه نشده، الان محاسبه کن
            baseMobility = calculateCustomP();
            if (!baseMobility) {
                console.warn('⚠️ لطفاً ابتدا درصدهای گروه مختلط را تکمیل کنید');
                return;
            }
        }
    } else {
        baseMobility = parseFloat(selectValue) || 2;
    }

    console.log(`📌 مقدار پایه p: ${baseMobility}`);

    // 2️⃣ بررسی چک‌باکس‌های شرایط اضافی
    let additionalPenalty = 0;
    const conditions = [];

    const perceptionLimited = document.getElementById('perception-limited');
    const noEvacuationPlan = document.getElementById('no-evacuation-plan');
    const panicRisk = document.getElementById('panic-risk');

    if (perceptionLimited && perceptionLimited.checked) {
        additionalPenalty += 2;
        conditions.push('افراد با درک محدود (+2)');
    }

    if (noEvacuationPlan && noEvacuationPlan.checked) {
        additionalPenalty += 2;
        conditions.push('عدم طرح تخلیه (+2)');
    }

    if (panicRisk && panicRisk.checked) {
        additionalPenalty += 2;
        conditions.push('خطر وحشت (+2)');
    }

    // 3️⃣ محاسبه p نهایی
    const totalP = baseMobility + additionalPenalty;
    console.log(`✅ p نهایی = ${baseMobility} + ${additionalPenalty} = ${totalP}`);

    // 4️⃣ ذخیره در calculationResults و متغیر سراسری
    calculationResults.p = totalP;
    window.mobilityFactorP = totalP; // ⭐ برای استفاده در calculateT

    // 5️⃣ نمایش نتیجه
    const finalResultDiv = document.getElementById('p-final-result');
    const detailsDiv = document.getElementById('p-calculation-details');
    
    if (finalResultDiv) {
        finalResultDiv.textContent = totalP.toFixed(2);
    }

    if (detailsDiv) {
        let detailsHTML = `
            <strong>مقدار پایه:</strong> ${baseMobility.toFixed(2)}<br>
        `;

        if (conditions.length > 0) {
            detailsHTML += `<strong>شرایط اضافی:</strong><br>`;
            conditions.forEach(c => {
                detailsHTML += `&nbsp;&nbsp;• ${c}<br>`;
            });
            detailsHTML += `<strong>جمع جریمه:</strong> +${additionalPenalty}<br>`;
        } else {
            detailsHTML += `<strong>شرایط اضافی:</strong> هیچ (+0)<br>`;
        }

        detailsHTML += `
            <hr style="border: none; border-top: 1px dashed #81c784; margin: 8px 0;">
            <strong style="color: #2e7d32; font-size: 1.1rem;">p نهایی = ${totalP.toFixed(2)}</strong>
        `;
    ...
    // 6️⃣ پیام
    if (conditions.length > 0) {
        showMessage('warning', `ضریب تحرک: ${totalP.toFixed(2)} (شامل ${conditions.length} شرط اضافی)`);
    } else {
        showMessage('success', `ضریب تحرک: p = ${totalP.toFixed(2)}`);
    }

    // 7️⃣ محاسبه مجدد t
    if (typeof calculateT === 'function') {
        calculateT();
    }
...
// =================== محاسبه ضریب t ===================
function calculateT() {
    // ✅ محاسبه x و K قبل از استفاده
    const manualX = parseFloat(document.getElementById('manual-x-input')?.value);
    let x;
    
    if (!isNaN(manualX) && manualX > 0) {
        x = manualX;
        calculationResults.x = x;
        console.log('📝 x دستی:', x);
    } else {
        x = calculateExitUnits();
    }
    
    calculateKfromPaths();
    const K = calculationResults.K;
    
    // دریافت سایر پارامترها
    const b = parseFloat(document.getElementById('section-length-t')?.value) || 0;
    const l = parseFloat(document.getElementById('section-width-t')?.value) || 0;
    const X = parseFloat(document.getElementById('occupants-count')?.value) || 0;
    
    // ⭐ استفاده از p نهایی (شامل شرایط اضافی)
    const p = window.mobilityFactorP || calculationResults.p || parseFloat(document.getElementById('mobility-factor')?.value) || 2;
    ...
    if (b <= 0 || l <= 0 || X <= 0 || x <= 0 || K <= 0) {
        showMessage('error', 'لطفاً همه مقادیر ضروری را وارد کنید');
        console.log('❌ مقادیر ناقص:', {b, l, X, x, K, p});
        return;
    }
    ...
    if (denominator <= 0) {
        showMessage('error', 'خطا در محاسبه: مخرج صفر یا منفی است');
        console.log('❌ مخرج:', denominator);
        return;
    }
    ...
    // ذخیره نتیجه
    calculationResults.t = t;

    // محاسبه RSET
    const RSET = t * 720;

    // نمایش نتایج میانی
    const intermediateDiv = document.getElementById('t-intermediate');
    if (intermediateDiv) {
        intermediateDiv.innerHTML = `
            <strong>📊 پارامترهای محاسبه:</strong><br>
            • طول بخش (l): ${l} متر<br>
            • عرض بخش (b): ${b} متر<br>
            • تعداد افراد (X): ${X} نفر<br>
            • واحدهای خروج (x): ${x.toFixed(2)}<br>
            • مسیرهای مجزا (K): ${K}<br>
            • ضریب تحرک (p): ${p.toFixed(2)}<br>
            • ارتفاع بالا (H⁺): ${Hplus} متر<br>
            • عمق پایین (H⁻): ${Hminus} متر<br>
            <br>
            <strong>🔢 محاسبات:</strong><br>
            • صورت: ${numerator.toFixed(4)}<br>
            • مخرج: ${denominator.toFixed(4)}<br>
            • ضرب در [x × (b+l)]: ${(x * (b + l)).toFixed(4)}
        `;
    }
    ...
    if (RSET < 180) {
        ...
        statusIcon = '✅';
        statusText = 'عالی - تخلیه سریع';
    } else if (RSET < 360) {
        statusClass = 'status-good';
        statusIcon = '✔️';
        statusText = 'خوب - زمان کافی';
    } else if (RSET < 600) {
        statusClass = 'status-acceptable';
        statusIcon = '⚠️';
        statusText = 'قابل قبول - نیاز به بررسی';
    } else if (RSET < 900) {
        statusClass = 'status-poor';
        statusIcon = '⚡';
        statusText = 'نامطلوب - نیاز به اصلاح';
    } else {
        statusClass = 'status-critical';
        statusIcon = '❌';
        statusText = 'بحرانی - خطر جدی';
    }
        ...
        statusDiv.innerHTML = `
            <div class="${statusClass}">
                ...
                <p style="font-size: 1.2em; margin: 10px 0;">
                    RSET = ${RSET.toFixed(1)} ثانیه (${(RSET / 60).toFixed(1)} دقیقه)
                </p>
            </div>
        `;
    }
    ...
    if (resultDiv) {
        resultDiv.innerHTML = `
            <h3 style="color: #667eea;">🎯 نتایج نهایی</h3>
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 10px; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.4);">
                <p style="font-size: 1.3em; margin: 10px 0;">
                    <strong>t (ضریب زمان تخلیه):</strong> ${t.toFixed(4)}
                </p>
                <p style="font-size: 1.5em; margin: 10px 0; font-weight: bold;">
                    <strong>RSET:</strong> ${RSET.toFixed(1)} ثانیه
                </p>
                <p style="font-size: 1.2em; margin: 10px 0;">
                    = ${(RSET / 60).toFixed(2)} دقیقه
                </p>
            </div>
        `;
    }
    ...
    // لاگ کنسول
    console.log('✅ calculateT - نتایج:');
    console.log('   p (نهایی) =', p.toFixed(2));
    console.log('   x =', x.toFixed(2));
    console.log('   K =', K);
    console.log('   t =', t.toFixed(4));
    console.log('   RSET =', RSET.toFixed(1), 'ثانیه');

    showMessage('success', 'ضریب زمان تخلیه محاسبه شد');
...
// =====================================================
// 📊 محاسبه خودکار تعداد افراد از ضریب تراکم
// =====================================================

function calculateOccupantsFromFactor() {
    console.log('🔵 calculateOccupantsFromFactor() شروع شد');
    
    const factor = parseFloat(document.getElementById('occupant-factor')?.value) || 0;
    let area = parseFloat(document.getElementById('section-area-t')?.value);
    
    if (!area || area <= 0) {
        const length = parseFloat(document.getElementById('section-length-t')?.value) || 0;
        const width = parseFloat(document.getElementById('section-width-t')?.value) || 0;
        
        if (length > 0 && width > 0) {
            area = length * width;
            document.getElementById('section-area-t').value = area.toFixed(2);
            console.log(`✅ مساحت محاسبه شد: ${area} m²`);
        }
    }
    
    if (factor > 0 && area > 0) {
        const X = Math.ceil(factor * area);
        document.getElementById('occupants-count').value = X;
        
        console.log(`✅ تعداد افراد محاسبه شد: ${X} نفر (ضریب: ${factor} × مساحت: ${area} m²)`);
        showMessage('success', `تعداد افراد: ${X} نفر (${factor} نفر/m² × ${area} m²)`);
        ...
        console.warn('⚠️ ضریب یا مساحت نامعتبر است');
        return null;
    }
}
...
function updateAreaFromDimensions() {
    ...
    if (length > 0 && width > 0) {
        ...
        document.getElementById('section-area-t').value = area.toFixed(2);
        console.log(`📐 مساحت به‌روز شد: ${area} m²`);
        calculateOccupantsFromFactor();
    }
}

// =================== Event Listeners ===================

// نمایش/مخفی کردن بخش گروه مختلط
document.getElementById('mobility-factor')?.addEventListener('change', function() {
    const customContainer = document.getElementById('custom-p-container');
...
// Listener برای چک‌باکس‌های شرایط اضافی
['perception-limited', 'no-evacuation-plan', 'panic-risk'].forEach(id => {
    document.getElementById(id)?.addEventListener('change', calculateMobilityFactor);
});

// Listener برای فیلدهای درصد گروه مختلط
['p-percent-a', 'p-percent-b', 'p-percent-c', 'p-percent-d'].forEach(id => {
    document.getElementById(id)?.addEventListener('input', () => {
        if (document.getElementById('mobility-factor').value === 'custom') {
            calculateCustomP();
        }
    });
});
...
function quickTestT() {
    document.getElementById('section-length-t').value = '20';
    document.getElementById('section-width-t').value = '16';
    document.getElementById('occupants-count').value = '32';
    document.getElementById('exit-widths').value = '80, 80';
    document.getElementById('external-exits').value = '2';
    document.getElementById('mobility-factor').value = '2';
    document.getElementById('height-above').value = '0';
    document.getElementById('height-below').value = '0';
    
    document.getElementById('manual-x-input').value = '';
    document.getElementById('manual-k-input').value = '';
    
    calculateMobilityFactor();
    calculateT();
    
    showMessage('success', 'تست مرجع بارگذاری شد (انتظار: x=2, K=2, RSET≈126s)');
}

function clearTInputs() {
    const fields = [
        'section-length-t', 'section-width-t', 'occupants-count',
        'exit-widths', 'manual-x-input', 'external-exits', 'manual-k-input',
        'mobility-factor', 'height-above', 'height-below',
        'p-percent-a', 'p-percent-b', 'p-percent-c', 'p-percent-d'
    ];
    ...
    ['t-intermediate', 't-status', 't-warnings', 't-result', 'exit-units-result', 'k-result', 'p-final-result', 'p-calculation-details', 'custom-p-value'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.innerHTML = '';
    });
    
    calculationResults.x = null;
    calculationResults.K = null;
    calculationResults.t = null;
    calculationResults.p = null;
    calculationResults.p_base = null;
    window.mobilityFactorP = null;
    
    showMessage('info', 'فیلدهای ورودی پاک شدند');
}

function exportTResults() {
    const results = {
        inputs: {
            section_length: parseFloat(document.getElementById('section-length-t')?.value) || 0,
            section_width: parseFloat(document.getElementById('section-width-t')?.value) || 0,
            occupants: parseFloat(document.getElementById('occupants-count')?.value) || 0,
            exit_widths: document.getElementById('exit-widths')?.value || '',
            external_exits: parseFloat(document.getElementById('external-exits')?.value) || 0,
            mobility_factor: window.mobilityFactorP || parseFloat(document.getElementById('mobility-factor')?.value) || 1,
            height_above: parseFloat(document.getElementById('height-above')?.value) || 0,
            height_below: parseFloat(document.getElementById('height-below')?.value) || 0
        },
        results: {
            x: calculationResults.x,
            K: calculationResults.K,
            p: calculationResults.p,
            t: calculationResults.t,
            RSET_seconds: calculationResults.t ? calculationResults.t * 720 : null,
            RSET_minutes: calculationResults.t ? (calculationResults.t * 720) / 60 : null
        },
    ...
    link.href = url;
    link.download = `FRAME_t_results_${Date.now()}.json`;
    link.click();
    URL.revokeObjectURL(url);
    
    showMessage('success', 'نتایج به فایل JSON صادر شد');
}

console.log('✅ سیستم ضریب تحرک با شرایط اضافی و Live Update فعال شد!');
...
// =====================================================
// 🔄 سیستم Live Update برای تب t
// =====================================================
// =====================================================
// 🔄 سیستم Live Update برای تب t (اصلاح شده)
// =====================================================
// =====================================================
// 🔄 سیستم Live Update برای تب t (نسخه کامل)
// =====================================================
document.addEventListener('DOMContentLoaded', function() {
    console.log('🟢 Live Update System برای تب t فعال شد');

    // لیست تمام ورودی‌هایی که باید محاسبات را trigger کنند
    const tInputIds = [
        'occupant-factor',      // ضریب تراکم افراد (SELECT)
        'occupants-count',      // تعداد افراد (دستی)
        'section-area-t',       // سطح بخش
        'exit-widths',          // عرض خروجی‌ها
        'manual-x-input',       // x دستی
        'manual-k-input',       // K دستی (اصلاح شده)
        'external-exits',       // تعداد خروجی‌های خارجی
        'mobility-factor',      // ضریب تحرک p
        'section-length-t',     // طول بخش (b)
        'section-width-t',      // عرض بخش (l)
        'height-above',         // ارتفاع بالای سطح زمین
        'height-below'          // ارتفاع پایین سطح زمین
    ];

    // تابع مرکزی برای محاسبه مجدد
    function recalculateAll() {
        console.log('🔄 محاسبه مجدد شروع شد...');
        
        // محاسبه X
        if (typeof calculateExitUnits === 'function') {
            calculateExitUnits();
        }
        
        // محاسبه K
        if (typeof calculateKfromPaths === 'function') {
            calculateKfromPaths();
        }
        ...
        console.log('✅ محاسبه مجدد تمام شد');
    }

    // اضافه کردن Event Listener به هر ورودی
    tInputIds.forEach(id => {
        const element = document.getElementById(id);
        if (element) {
            const tagName = element.tagName.toLowerCase();
            
            // برای SELECT (منوی کشویی)
            if (tagName === 'select') {
                element.addEventListener('change', function() {
                    console.log(`📊 ${id} تغییر کرد:`, element.value);
                    
                    // 🔥 برای منوی ضریب تراکم، ابتدا تعداد افراد را محاسبه کن
                    if (id === 'occupant-factor') {
                        calculateOccupantsFromFactor();
                    } else {
                        recalculateAll();
                    }
                });
                console.log(`✅ Listener (SELECT) به ${id} اضافه شد`);
            }
            // برای INPUT (فیلدهای متنی)
            else if (tagName === 'input') {
                element.addEventListener('input', function() {
                    // برای طول و عرض، ابتدا مساحت را به‌روز کن
                    if (id === 'section-length-t' || id === 'section-width-t') {
                ...
                console.log(`✅ Listener (INPUT) به ${id} اضافه شد`);
            }
        } else {
            console.warn(`⚠️ عنصر ${id} پیدا نشد`);
        }
    });

    // اضافه کردن Listener به Radio Button‌های X و K
    const xRadios = document.querySelectorAll('input[name="x-mode"]');
    xRadios.forEach(radio => {
        radio.addEventListener('change', recalculateAll);
    });

    const kRadios = document.querySelectorAll('input[name="k-mode"]');
    kRadios.forEach(radio => {
        radio.addEventListener('change', recalculateAll);
    });

    console.log('🎉 Live Update System آماده است!');
});


    // =====================================================
    // 🔄 Live Update برای ضریب تحرک p
    // =====================================================
    
    // Listener برای چک‌باکس‌های شرایط اضافی
    const pCheckboxIds = ['perception-limited', 'no-evacuation-plan', 'panic-risk'];
    pCheckboxIds.forEach(id => {
        const checkbox = document.getElementById(id);
        if (checkbox) {
            checkbox.addEventListener('change', function() {
                console.log(`🔔 چک‌باکس ${id} تغییر کرد`);
                calculateMobilityFactor();
            });
            console.log(`✅ Listener (CHECKBOX) به ${id} اضافه شد`);
        }
    });

    // Listener برای فیلدهای درصد گروه مختلط
    const pPercentIds = ['p-percent-a', 'p-percent-b', 'p-percent-c', 'p-percent-d'];
    pPercentIds.forEach(id => {
        const input = document.getElementById(id);
        if (input) {
            input.addEventListener('input', function() {
                const selectValue = document.getElementById('mobility-factor')?.value;
                if (selectValue === 'custom') {
                    console.log(`🔢 درصد ${id} تغییر کرد`);
                    calculateCustomP();
                    calculateMobilityFactor();
                }
            });
    ...
    console.log('🎉 Live Update برای ضریب تحرک p فعال شد!');
