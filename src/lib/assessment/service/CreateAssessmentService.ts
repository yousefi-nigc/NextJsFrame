...
export async function createAssessmentService({
    ...
    try {
        ...
        // Create assessment with inputs and calculated values in separate tables using transaction
        const assessment = await db.$transaction(async (tx) => {
            // Create assessment with inputs only
            ...
            // Create calculated value tables
            ...
            await tx.assessmentPotentialRisks.create({
                data: {
                    assessmentId: assessment.id,
            ...
            await tx.assessmentAcceptableLevels.create({
                data: {
                    assessmentId: assessment.id,
            ...
            await tx.assessmentProtectionLevels.create({
                data: {
                    assessmentId: assessment.id,
